    ...
    internal func getToken(_ url: String = PXServicesURLConfigs.MP_CREATE_TOKEN_URI, cardTokenJSON: Data, success: @escaping (_ data: Data) -> Void, failure:  ((_ error: PXError) -> Void)?) {
        ...
        self.request(uri: url, params: params, body: cardTokenJSON, method: HTTPMethod.post, success: success, failure: { (error) -> Void in
            if let failure = failure {
                failure(PXError(domain: ApiDomains.GET_TOKEN, code: error.code, userInfo: [NSLocalizedDescriptionKey: "Hubo un error", NSLocalizedFailureReasonErrorKey: "Verifique su conexión a internet e intente nuevamente"]))
            }
        })
    }
            ...
            self.request(uri: url + "/" + tokenId + "/clone", params: "public_key=" + public_key, body: nil, method: HTTPMethod.post, success: { (data) in
                do {
                    ...
                    self.request(uri: url + "/" + token!.id, params: "public_key=" + public_key, body: jsonData, method: HTTPMethod.put, success: success, failure: { (_) in
                        failure?(PXError(domain: ApiDomains.CLONE_TOKEN, code: ErrorTypes.NO_INTERNET_ERROR, userInfo: [NSLocalizedDescriptionKey: "Hubo un error", NSLocalizedFailureReasonErrorKey: "Verifique su conexión a internet e intente nuevamente"]))
                    })
                } catch {
                ...
                if let failure = failure {
                    failure(PXError(domain: ApiDomains.CLONE_TOKEN, code: error.code, userInfo: [NSLocalizedDescriptionKey: "Hubo un error", NSLocalizedFailureReasonErrorKey: "Verifique su conexión a internet e intente nuevamente"]))
                }
            })
        }
    }
